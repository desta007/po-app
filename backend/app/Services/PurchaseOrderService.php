...
class PurchaseOrderService
{
    ...
    public function create(array $data, array $items): PurchaseOrder
    {
        return DB::transaction(function () use ($data, $items) {
            ...
            $calculated = $this->calculateTotals($items, $data['discount'] ?? 0, $data['tax'] ?? 0);
    ...
    public function update(PurchaseOrder $po, array $data, array $items): PurchaseOrder
    {
        return DB::transaction(function () use ($po, $data, $items) {
            $calculated = $this->calculateTotals($items, $data['discount'] ?? $po->discount, $data['tax'] ?? $po->tax);
            $data = array_merge($data, $calculated);
    ...
    public function calculateTotals(array $items, float $discount = 0, float $tax = 0): array
    {
